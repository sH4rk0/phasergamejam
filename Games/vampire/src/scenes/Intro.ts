export default class Intro extends Phaser.Scene {
  ...
  //variabile per la tilesprite dello sfondo
  private _cubes2: Phaser.GameObjects.TileSprite;
  //in questa variabile inizializzeremo la musica dell'introduzione 
  private _music: Phaser.Sound.BaseSound;

  private _gear1: Phaser.GameObjects.Sprite;
  private _gear2: Phaser.GameObjects.Sprite;
  private _gear3: Phaser.GameObjects.Sprite;
  ...
  create() {
    ...
    //creo l'istanza della tilesprite dello sfondo
    this._cubes2 = this.add.tileSprite(0, 0, 1024, 600, "cubes2").setOrigin(0).setAlpha(1);


    this._gear2 = this.add.sprite(0, 410, "gears").setFrame(1).setTintFill(0x333333).setBlendMode("MULTIPLY").setAlpha(.5);
    this._gear1 = this.add.sprite(0, 0, "gears").setFrame(1).setTintFill(0x333333).setBlendMode("MULTIPLY").setAlpha(.5)
    //this._gear3 = this.add.sprite(0, 0, "gears").setFrame(2);
    ...
    this._logo = this.add.image(this.game.canvas.width / 2, 150, "greenbots").setAlpha(0).setScale(1);

    //creo il primo tween che porterà il logo dalla posizione y:50 alla posizione y:150 e dall'alpha 0 all'alpha 1
    this.add.tween({
      targets: this._logo, y: 200, alpha: 1, duration: 1000, ease: "quad.easeInOut",
      onComplete: () => {
    ...
    this._play = this.add
      .bitmapText(this.game.canvas.width / 2, 460, "arcade", "PLAY")
    ...
    this._howToPlay = this.add
      .bitmapText(this.game.canvas.width / 2, 550, "arcade", "Come giocare", 20)
    ...
    this._credits = this.add
      .bitmapText(this.game.canvas.width / 2, 600, "arcade", "Crediti", 20)
      .setAlpha(0) //setto l'alpha a 1
      .setOrigin(0.5) //setto l'origin centrale
      .setInteractive() //abilito l'interazione 
      .setDepth(100) //setto la profondità
  ...
  update(time: number, delta: number): void {
    ...
    //muovo le tile con moto circolare usanto seno e coseno
    this._cubes2.tilePositionX += Math.sin(this._counter2) //* 3.5; //moltiplicatore per la velocità;
    this._cubes2.tilePositionY += Math.cos(this._counter2) //* 3.5; //moltiplicatore per la velocità
  }
